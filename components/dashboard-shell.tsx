    ...
    <>
      ...
      <main className="min-h-[calc(100vh-4rem)] bg-surface">
        <div className="container-page grid gap-6 py-8 lg:grid-cols-[260px_1fr]">
          <aside className="h-fit rounded-lg border border-slate-200 bg-white p-3 shadow-card lg:sticky lg:top-20">
              ...
              {nav.map((item) => {
                ...
                return (
                  <a
                    key={item}
                    href={`#${slugify(item)}`}
                    className="flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-slate-600 hover:bg-surface hover:text-primary"
                  >
